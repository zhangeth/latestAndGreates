...
function main() {
  ...
  fs.readFile(inputFile, 'utf8', (err, data) => {
    ...
    modifiedData = modifiedData.replace(/const JSCHEMA = ""/, `const JSCHEMA = ${JSON.stringify(JSchema)}`);
    modifiedData = modifiedData.replace(/const UISCHEMA = ""/, `const UISCHEMA = ${JSON.stringify(UISchema)}`);
    modifiedData = modifiedData.replace(/const FILEPATHS = ""/, `const FILEPATHS = ${JSON.stringify(pathsWithFileExtension)}`);
